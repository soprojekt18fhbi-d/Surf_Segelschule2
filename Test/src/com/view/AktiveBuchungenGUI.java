	...
	public AktiveBuchungenGUI(IObjektModel models, BuchungBuchungAnzeigenStrg controllers) {
					...
					if (buchungID == 0 && kNr == 0)
						...
						if (mode.equals("Verkauf")) {
							int ergebnis = JOptionPane.showConfirmDialog(null,
									"Rechnung wirklich erstellen? Sie wird dann nicht mehr als aktive Buchung angezeigt!");
							if (ergebnis != 0)
		...
		btnStorn.addActionListener(new ActionListener() {
			...
			private int nachfrage(JComboBox modusBox) {
				int wert = JOptionPane.showConfirmDialog(null, modusBox.getSelectedItem().toString() + " stornieren?",
						"Warnung!", JOptionPane.YES_NO_OPTION);
				if (wert == 0) {

					JOptionPane.showMessageDialog(null,
							modusBox.getSelectedItem().toString() + " wurde erfolgreich storniert", "Warnung!",
							JOptionPane.PLAIN_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null,
							modusBox.getSelectedItem().toString() + " wurde nicht storniert", "Warnung!",
							JOptionPane.PLAIN_MESSAGE);
				}
				return wert;
			}

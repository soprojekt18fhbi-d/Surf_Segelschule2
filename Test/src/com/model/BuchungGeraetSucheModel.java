...
public class BuchungGeraetSucheModel implements IObjektModel { // @author Ben Kröncke
	...
	public void viewTable(Connection con) throws SQLException {
		...
		String query = "select ID,MAKEL,VERKAUFSPREIS, FARBE from SPORTGERAET where MODELLID = '" + modellNr
				+ "' AND STATUS = 'OK' AND STANDORTID = " + standort;
		String update = null;
	...
	private void erzeugeGeraetTabelle(Statement stmt) throws SQLException {
		...
		// Die Sucheingabe wurde als "modellNr" übergeben, damit methode nicht mehr
		// verändert werden muss!

		query = "select ID,MAKEL,VERKAUFSPREIS, FARBE from SPORTGERAET where MODELLID = '" + modellNr
				+ "' AND STATUS = 'OK' AND STANDORTID = " + standort;
		if (talking.equals("gesamt"))
			query = "select * from SPORTGERAET";
		if (modellNr == 123456789) {
			String textEingabe = talking;
			query = "select * from SPORTGERAET WHERE ID LIKE '" + textEingabe + "%' OR MAKEL LIKE '" + textEingabe
					+ "%' OR TYPID LIKE '" + textEingabe + "%' OR MODELLID LIKE '" + textEingabe + "%' OR STATUS LIKE '"
					+ textEingabe + "%' OR FARBE LIKE '" + textEingabe + "%' OR BAUJAHR LIKE '" + textEingabe
					+ "%' OR VERKAUFSPREIS LIKE '" + textEingabe + "%' OR ANSCHAFFUNGSPREIS LIKE '" + textEingabe
					+ "%'";
		}
		System.out.println(query);

		ResultSet rs = stmt.executeQuery(query);
		table = DbUtils.resultSetToTableModel(rs); // abgefragets ResultSet wird umgewandelt in ein TableModel und hier
													// in der Variable table gespeichert
	}

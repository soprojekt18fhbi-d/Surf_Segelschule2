...
public class BuchungAnzeigeModel implements IObjektModel { // Ben Kröncke
	...
	public void erstelleRechnungVerkauf(Statement stmt) throws SQLException {
		...
		update = "INSERT INTO RECHNUNG VALUES( default, " + (verkaufspreis / 1.19) + ", " + (verkaufspreis / 119) * 19
				+ ", " + verkaufspreis + ", " + kNr + ", null, " + buchungID + ")";
	...
	public void vollendeAusleihe(Statement stmt, String date) throws SQLException {
		...
		update = "INSERT INTO RECHNUNG VALUES (default," + preis / 1.19 + ", " + (preis / 1.19) * 0.19 + ", " + preis
				+ ", " + kNr + ", null, " + buchungID + ");";
	...
	public void pruefepreisSetzen(Statement stmt, int[] tageStunden) throws SQLException {
		...
		if (tageStunden[0] <= 1 && tageStunden[1] > 4) {
	...
	public double holeBonuspreis(Statement stmt, String spalte) throws SQLException {
		...
		ResultSet rs;
		double bonus = 0;
		query = "SELECT PREISLISTE." + spalte + " FROM PREISLISTE, MODELL, SPORTGERAET WHERE SPORTGERAET.ID = "
				+ geraetNrloc + " AND SPORTGERAET.MODELLID = MODELL.ID AND MODELL.PREISLISTEID = PREISLISTE.ID";
	...
	public int[] holeTageStunden(String rueckdatum, String verleihdatum) {
		...
		// Splitten der Daten in unterschiedliche Parts, um Integer auslesen zu können,
		// und damit die Dauer des Verleihs zu berechnen.
	...
	public int[] berechneTageStunden(String[] datumRueck, String[] uhrzeitRueck, String[] datumVerleih,
		...
		if (minuten > 0) {
			stunden++; // Wenn die Minuten den Wert 0 übersteigen (eine neue Stunde angefangen wurde,
						// dann wird der Stundencounter um 1 erhöht
		}

		int monate = (Integer.parseInt(datumRueck[1]) - Integer.parseInt(datumVerleih[1]));
		if (monate < 0)
			monate = ((12 - Integer.parseInt(datumVerleih[1])) + Integer.parseInt(datumRueck[1])); // Überprüfen, ob
																									// Monate anständig
																									// ausgelesen
																									// worden. Wenn
																									// nicht, dann wird
		...
		if (tage < 0) {
			monate--;
			tage = ((30 - Integer.parseInt(datumVerleih[2])) + Integer.parseInt(datumRueck[2])); // Überprüfen, ob Tage
																									// anständig
																									// ausgelesen
																									// worden. Wenn
																									// nicht, dann wird
	...
	public void sucheVerkaeufe(Statement stmt) throws SQLException {
		String query;
		query = "SELECT ID,KUNDEID,SPORTGERAETID,VERKAUFSDATUM FROM BUCHUNG WHERE ART = 'Verkauf' AND VERKAUFSDATUM LIKE '%"
				+ search + "%'";
		ResultSet rs = stmt.executeQuery(query);
	...
	public void setVars(String talking2, int buchungID2, int kNr2, int geraetNr, String search2, String mode2,
			String makel2) {
		talking = talking2;
		kNr = kNr2;
